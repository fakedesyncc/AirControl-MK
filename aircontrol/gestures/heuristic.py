...
class HeuristicPoseClassifier:
    ...
    def predict(self, landmarks: np.ndarray):
        index_up = F.finger_extended(landmarks, INDEX_TIP, 1.5)
        middle_up = F.finger_extended(landmarks, MIDDLE_TIP, 1.5)
        ring_up = F.finger_extended(landmarks, RING_TIP, 1.5)
        pinky_up = F.finger_extended(landmarks, PINKY_TIP, 1.5)

        index_fold = F.finger_folded(landmarks, INDEX_TIP, 1.3)
        middle_fold = F.finger_folded(landmarks, MIDDLE_TIP, 1.3)
        ring_fold = F.finger_folded(landmarks, RING_TIP, 1.3)
        pinky_fold = F.finger_folded(landmarks, PINKY_TIP, 1.3)
        ...
        strict_open = sum([F.finger_extended(landmarks, t, 1.45)
