...
```mermaid
flowchart LR
    Camera["Camera"] --> Tracker["MediaPipe HandTracker"]
    Tracker --> GestureEngine["GestureEngine"]
    GestureEngine --> Fusion["MultimodalCoordinator"]
    Voice["VoiceRecognizer"] --> Fusion
    Fusion --> Cursor["CursorController"]
    Fusion --> Actions["ActionExecutor"]
    Cursor --> InputBackend["OS input backend"]
    Actions --> InputBackend
...
| --- | --- | --- |
| `tracking/` | камера, MediaPipe Hand Landmarker, фильтры | можно тестировать трекинг отдельно от GUI |
| `gestures/` | признаки, позы, щипки, one-gesture режим | распознавание не исполняет действия напрямую |
| `fusion/` | разрешение конфликтов жестов, курсора и голоса | одно место для правил приоритета |
