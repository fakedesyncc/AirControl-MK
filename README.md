...
<p align="center">
  <a href="https://github.com/fakedesyncc/AirControl-MK/releases/latest"><b>Скачать релиз</b></a>
  &nbsp;·&nbsp;
...
Текущая ветка `main` содержит стабильный product build. Разработка идёт через
небольшие `feature/*`, `docs/*` и `release/*` ветки; `develop` отражает состояние
для следующей интеграции, `deploy` указывает на ревизию для упаковки.

| Канал       | Назначение                                      |
| ----------- | ----------------------------------------------- |
| `main`      | стабильная ветка с готовым приложением          |
| `develop`   | интеграционная ветка для следующего цикла       |
| `release/*` | подготовка версий и релизных правок             |
| `feature/*` | отдельные продуктовые возможности               |
| `docs/*`    | документация, методика, оформление репозитория  |
| `deploy`    | состояние, которое можно отправлять на упаковку |
...
Подробнее — [docs/THESIS.md](docs/THESIS.md), раздел 3.3.1.
