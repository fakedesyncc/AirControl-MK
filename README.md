...
| Ассистивное управление | перед запуском проверяет input backend, затем включает курсор и dwell-click |
...
dwell-click. В окне управления можно переключать профиль удержания: `fast` для
быстрого клика, `normal` для обычной работы, `steady` для медленного уверенного
клика с большей зоной удержания.
